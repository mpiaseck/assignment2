...
(define (maybe-play-chunk cur w)
  (local [(define next-start (world1-next-start w))]
    (cond [(time-to-play? cur next-start)
           (local [(define playhead (if (< (world1-play-head w) END-TIME1) (world1-play-head w) 0))
...
(define (draw-world w)
  (draw-play
   ...
   (place-image (rectangle 10 100 "solid" "black")
                (+ 150
                   (* SLIDER-WIDTH (/ (world1-play-head w) (rs-frames snd1))))
                100
...
(define (draw-play w scene)
  ...
  ; change the worlds playing boolean values respectively once we get the
  ; nested structures to work. This will have the circles drawn appropriately                                                                         
       (place-image (circle 50 (if (not (world1-playing? w)) "outline" "solid") 
